...
const tekrarSayilariObj = {};

for (let i = 0; i < sayilar.length; i++) {
  const sayi = sayilar[i];
  if (tekrarSayilariObj[sayi] === undefined) {
    tekrarSayilariObj[sayi] = 1;
  } else {
    tekrarSayilariObj[sayi]++;
  }
}

tekraredensayilar = [];
for (const sayi in tekrarSayilariObj) {
  if (tekrarSayilariObj[sayi] > 1) {
    tekraredensayilar.push(
      `${sayi} sayısı ${tekrarSayilariObj[sayi]} kere tekrar edilmiştir`
    );
  }
}
